...
int main(int argc, char *argv[]) {
	...
	do {
		...
		printf("hw:    %#7.2f %cJ   sys:   %#7.2f %cJ   proc:  %#7.2f %cJ   sleep: %#7.2f %cJ\n", 
				cp->hw_d.battery_SOC, cp->hw_p.battery_SOC, 
				cp->sys_d.battery_SOC, cp->sys_p.battery_SOC, 
				cp->proc_d.battery_SOC, cp->proc_p.battery_SOC,
				cp->hw_d.battery_SOC, cp->hw_p.battery_SOC);
				
		printf("delta: %#7.2f %cJ   delta: %#7.2f %cJ   delta: %#7.2f %cJ   delta: %#7.2f %cJ\n", 
				dp->hw_d.battery_SOC, dp->hw_p.battery_SOC, 
				dp->sys_d.battery_SOC, dp->sys_p.battery_SOC, 
				dp->proc_d.battery_SOC, dp->proc_p.battery_SOC,	
				dp->hw_d.battery_SOC, dp->hw_p.battery_SOC);
		printf("--------------------------------------------------------------------------------\n");
