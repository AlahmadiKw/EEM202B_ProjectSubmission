...
extern uint64_t glob_alpha;
extern uint64_t glob_charge;
...
//				last_state.variables.cycles[i] = vemu_get_cycles(i);
			...
			// the first two elemtens in cycles[] are replaced with battery information
			// the user will not be able to quary for cycles information since they're replaced
			// with battery information 
		        last_state.variables.cycles[0] = glob_alpha;
		        last_state.variables.cycles[1] = glob_charge;
